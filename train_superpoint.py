...
if config['data']['generate_label']:
    ...
    while n_iter < max_iter:
        ...
        for count, sample in enumerate(tqdm.tqdm(train_loader)):
            ...
            # plt.show()
            loss = Net.train_mode(sample)
            loss.backward()
            running_loss += loss.item()
            optimizer.step()
        print(f'Training Loss for the epoch-{n_iter}: {running_loss}')
        val_loss = 0
        for count_val, sample_val in tqdm.tqdm(enumerate(val_loader)):
            with torch.no_grad():
                val_loss += Net.train_mode(sample_val).item()
        print(f'Val Loss for the epoch-{n_iter}: {val_loss}')
